...
async def test_remediation_job_marks_finding_resolved(monkeypatch, tmp_path: Path):
    ...
    async def fake_relink_finding(session, finding, *, allow_library_lookup=True):
...
@pytest.mark.asyncio
async def test_remediation_uses_lightweight_relink_for_already_linked_findings(monkeypatch, tmp_path: Path):
    media_file = tmp_path / "Linked.Show.S01E02.mkv"
    media_file.write_bytes(b"x" * 300_000)
    relink_calls = []
    ...
    async def fake_relink_finding(session, finding, *, allow_library_lookup=True):
        relink_calls.append(allow_library_lookup)
        return SimpleNamespace(
            manager_kind=ManagerKind.SONARR,
            manager_entity_id="episode:135477",
            title="Linked Show",
            season_number=1,
            episode_number=2,
            year=None,
        ...
        async with session.begin():
            ...
            session.add(
                RemediationJob(
                    ...
                    status="queued",
                    attempt_count=0,
                    requested_by="test-lightweight-relink",
                )
            )
    ...
    async with SessionLocal() as session:
        async with session.begin():
            job = (
                await session.execute(
                    select(RemediationJob)
                    .where(RemediationJob.requested_by == "test-lightweight-relink")
                    .order_by(RemediationJob.id.desc())
                )
            ).scalar_one()
            await execute_job(session, job.id, actor="test")
...
@pytest.mark.asyncio
async def test_remediation_surfaces_timeout_errors_with_readable_message(monkeypatch, tmp_path: Path):
    media_file = tmp_path / "Timed.Out.Movie.2022.mkv"
    media_file.write_bytes(b"x" * 300_000)
        ...
        async with session.begin():
            ...
            session.add(
                RemediationJob(
                    ...
                    status="queued",
                    attempt_count=0,
                    requested_by="test-timeout-readable",
                )
            )
    ...
    async with SessionLocal() as session:
        async with session.begin():
            job = (
                await session.execute(
                    select(RemediationJob)
                    .where(RemediationJob.requested_by == "test-timeout-readable")
                    .order_by(RemediationJob.id.desc())
                )
            ).scalar_one()
            await execute_job(session, job.id, actor="test")
    ...
    async with SessionLocal() as session:
        ...
        assert job.status == JobStatus.FAILED.value
        assert job.last_error == "Manager request timed out"
        assert attempts[0].response_summary
        assert "Manager request timed out" in attempts[0].response_summary
